...
class SRGAN():
    ...
    def train(self, epochs, init_epoch=0, batch_size=1, sample_interval=50):
        ...
        for epoch in range(init_epoch, epochs):
            ...
            # image_features.shape [1,256,512,512]
    ...
    def sample_images(self, epoch):
        os.makedirs('srgan/%s' % self.dataset_name, exist_ok=True)
        r, c = 2, 2
        ...
        for row in range(r):
            ...
            cnt += 1
        fig.savefig("srgan/%s/%d.png" % (self.dataset_name, epoch))
        plt.close()

        for i in range(r):
            fig = plt.figure()
            plt.imshow(imgs_lr[i])
            fig.savefig('srgan/%s/%d_lowres%d.png' % (self.dataset_name, epoch, i))
            plt.close()

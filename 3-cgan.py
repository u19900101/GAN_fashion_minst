...
from keras.layers import BatchNormalization, Activation, ZeroPadding2D, multiply, Embedding
...
class CGAN():
    ...
    def train(self, epochs, batch_size=128, sample_interval=50):
        ...
        for epoch in range(epochs):
            ...
            if epoch % 5 == 0:
                logs.append([epoch, d_loss[0], 100 * d_loss[3], g_loss[0]])
        self.showlogs(logs)
    ...
    def sample_images(self, epoch):
        ...
        for i in range(r):
            for j in range(c):
                ...
                axs[i, j].axis('off')
                cnt += 1
        fig.savefig("fashion_mnist_cgan/%d.png" % epoch)
        plt.close()


if __name__ == '__main__':
    if not os.path.exists("./fashion_mnist_cgan"):
        os.makedirs("./fashion_mnist_cgan")
    start = time.time()
    gan = CGAN()
    epochs = 3000
    ...
    file.write("CGAN epochs : "+str(epochs)+" "+curT+str(end-start)+'\n')
    file.close()